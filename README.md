# Parallax-v1 (Windows setup)

### How to use:

1. Run `ParallaxOverlay.exe` (double-click to launch).
2. Press **Ctrl + Space** to show/hide the Parallax viewport anywhere, on any virtual desktop.
3. Use the **[ X ]** in the top right to close the program.

### For the Browser:

1. Click **<** to go Back
2. Click **>** to go Forward
3. Click **R** to Refresh

## Important Note

Do **not** move or delete the `.dll` files relative to `ParallaxOverlay.exe`. Executables rely on these dynamic link libraries at runtime, so they must always remain in the exact same directory.

> **Note:** If Windows SmartScreen shows a *"Windows protected your PC"* warning, click **More info** and then click **Run anyway**.

## Support

If this project helped you or you liked it, please give it a ⭐ star on GitHub! It helps the project grow and reach more people.

For any bugs, queries, or suggestions please do reach out :)
